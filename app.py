...
CAMERA_SPECS = {
    "RGB": {
        ...
        "image_width_px": 5280,
        "min_write_time_s": 1.0,  # 1 mp fotók közti minimum idő RGB-nek
    ...
    "Multispektrális": {
        ...
        "sensor_width_mm": 6.4,
        "image_width_px": 1600,
        "min_write_time_s": 2.0,  # 2 mp fotók közti minimum idő Multi-nak
        "correction_factor": 0.877
    }
}

# Konstansok
MAX_DRONE_SPEED = 15.0  # m/s, maximális drón sebesség
SAFETY_SPEED_FACTOR = 0.9  # 10% biztonsági ráhagyás a sebességen
AKKU_FLIGHT_TIME_MIN = 20  # perc / akku
...
def calculate_flight_time(area_ha, side_overlap_pct, front_overlap_pct, speed_mps, camera_type):
    specs = CAMERA_SPECS[camera_type]
    area_m2 = area_ha * 10000
    sensor_width_m = calculate_flight_altitude(1, camera_type) * (specs["sensor_width_mm"] / specs["focal_length_mm"]) / 1000 * specs["image_width_px"] / specs["image_width_px"]  # approx
    side_footprint = sensor_width_m * (1 - side_overlap_pct / 100)
    rows = math.ceil(math.sqrt(area_m2) / side_footprint)
    row_length = area_m2 / (rows * side_footprint)
    total_distance = rows * row_length
    flight_time_sec = total_distance / speed_mps
    return flight_time_sec / 60  # perc
...
planned_flight_time = st.number_input("Tervezett repülési idő (percben):", min_value=1.0, value=20.0, step=0.1)
...
if st.button("Számítás indítása"):
    ...
    flight_time = calculate_flight_time(area_ha, side_overlap_pct, front_overlap_pct, max_speed, priority)
    ...
    st.info(f"Becsült repülési idő a megadott területre: {flight_time:.1f} perc")

...
st.title("hRepüléstervező kalkulátor")
...
KORREKCIO_RGB_REPMAG = 2
KORREKCIO_MULTI_GSD = 1 / 0.56
...
if st.button("▶️ Számítás indítása"):
    ...
    # RGB repülési magasság számítása
    alap_repmag_cm = (gsd_cm * rgb["fokusz_mm"] * rgb["képszélesség_px"]) / rgb["szenzor_szelesseg_mm"]
    rgb_repmag_m = alap_repmag_cm / 100 / KORREKCIO_RGB_REPMAG

    eredmenyek = [("RGB", szamol(rgb, rgb_repmag_m, side_overlap_pct))]

    if kamera_mod == "RGB + multispektrális":
        eredmenyek.append(("Multispektrális", szamol(multi, rgb_repmag_m, side_overlap_pct)))
    ...
    for nev, eredeti in eredmenyek:
        ...
        if nev == "RGB":
            ...
            gsd_multi_cm = gsd_cm * KORREKCIO_MULTI_GSD

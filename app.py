...
# Minimum GSD számítása a multispektrális adatok alapján
...
def szamol(kamera, repmag_m, side_overlap_val, shutter_speed):
    kep_szelesseg_m = repmag_m * kamera["szenzor_szelesseg_mm"] / kamera["fokusz_mm"]
    savszel_m = kep_szelesseg_m * (1 - side_overlap_val / 100)

    gsd_cm_val = (repmag_m * kamera["szenzor_szelesseg_mm"]) / (kamera["fokusz_mm"] * kamera["képszélesség_px"]) * 100

    vmax_blur = (gsd_cm_val / 100) * MAX_PIXEL_ELMOZDULAS / shutter_speed
    vmax_write = (gsd_cm_val / 100) * kamera["képszélesség_px"] / kamera["min_írási_idő_s"]
...
if st.button("▶️ Számítás indítása"):
    ...
    # RGB repmag számítás (klasszikus képlet / 2)
    rgb = available_drones[selected_drone_name]["RGB"]
    repmag_cm_rgb = (gsd_cm * rgb["fokusz_mm"] * rgb["képszélesség_px"]) / rgb["szenzor_szelesseg_mm"]
    repmag_m_rgb = repmag_cm_rgb / 100 / 2

    eredmenyek = [("RGB", szamol(rgb, repmag_m_rgb, side_overlap_pct, shutter_speed))]

    if kamera_mod == "RGB + multispektrális":
        # Multi GSD számítása: RGB GSD × 1 / 0.56
        gsd_multi_cm = gsd_cm * (1 / 0.56)
        multi_repmag_cm = (gsd_multi_cm * multi["fokusz_mm"] * multi["képszélesség_px"]) / multi["szenzor_szelesseg_mm"]
        multi_repmag_m = multi_repmag_cm / 100
        eredmenyek.append(("Multispektrális", szamol(multi, multi_repmag_m, side_overlap_pct, shutter_speed)))
    ...
    for nev, eredeti in eredmenyek:
        ...
        if nev == "Multispektrális":
            multi_gsd = gsd_cm * (1 / 0.56)
